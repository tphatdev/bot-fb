...
// Kiểm tra xem resData có phần tử không
        if (resData.length > 0) {
            // Kiểm tra xem phần tử cuối cùng của resData có thuộc tính 'successful_results' không
            if (resData[resData.length - 1].successful_results === 0) {
                throw { error: "getSeqId: there was no successful_results", res: resData };
            } else {
                // Xử lý logic khi successful_results được xác định
            }
        } else {
            // Xử lý logic khi resData không có phần tử
        }
